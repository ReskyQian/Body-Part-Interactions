...
if __name__ == "__main__":
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            npz_data = np.load('/media/resky/Datasets/ntu120/NTU120_CSub.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xset' in arg.dataset:
            npz_data = np.load('/media/resky/Datasets/ntu120/NTU120_CSet.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'ntu' in arg.dataset:
        if 'xsub' in arg.dataset:
            npz_data = np.load('/media/resky/Datasets/NTU60_CS.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in arg.dataset:
            npz_data = np.load('/media/resky/Datasets/NTU60_CV.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    else:
