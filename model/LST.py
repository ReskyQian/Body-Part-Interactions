...
class Model_lst_structure(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, head=['ViT-B/32'], k=1):
        super(Model_lst_structure, self).__init__()
        ...
        (num_node, left_index, right_index, torso, arm, leg) = self.graph.symmetry_info
    ...
    def forward(self, x):
        ...
        torso_feature = self.part_list[0](feature[:,:,:,:,self.torso].mean(4).mean(3).mean(1))
        arm_feature = self.part_list[1](feature[:,:,:,:,self.arm].mean(4).mean(3).mean(1))
        leg_feature = self.part_list[2](feature[:,:,:,:,self.leg].mean(4).mean(3).mean(1))
        ...
        return self.fc(x), feature_dict, self.logit_scale.exp(), [torso_feature, arm_feature, leg_feature]
