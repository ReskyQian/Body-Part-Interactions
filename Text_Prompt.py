...
def text_prompt_openai_pasta_pool_4part():
    ...
    for ii in range(num_text_aug):
        if ii == 0:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0]) for pasta_list in paste_text_map2])
        elif ii == 1:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[1] + ',' + pasta_list[4]) for pasta_list in paste_text_map2])
        elif ii == 2:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[2] + ',' + pasta_list[3]) for pasta_list in paste_text_map2])
        elif ii == 3:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[5] + ',' + pasta_list[6]) for pasta_list in paste_text_map2])
...
# 2 代表 双手 + 双臂
# 3 代表 双腿 + 双脚
def text_prompt_openai_pasta_pool_symmetry_ucla():
    print("Use text prompt openai pasta pool ucla")
    text_dict = {}
    num_text_aug = 4
    ...
    for ii in range(num_text_aug):
        if ii == 0:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0]) for pasta_list in ucla_paste_text_map1])
        elif ii == 1:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[1] + ',' + pasta_list[4]) for pasta_list in ucla_paste_text_map1])
        elif ii == 2:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[2] + ',' + pasta_list[3]) for pasta_list in ucla_paste_text_map1])
        elif ii == 3:
            text_dict[ii] = torch.cat([clip.tokenize(pasta_list[0] + ',' + pasta_list[5] + ',' + pasta_list[6]) for pasta_list in ucla_paste_text_map1])
